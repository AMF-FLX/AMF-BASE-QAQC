...
class WindroseAnalysis:
    ...
    def calculate_wind_frequency(self,
        ...
        # count: Matrix for storing frequency of wind speeds within
        # each direction bin, for each wind speed interval.
    ...
    def generate_frequency_table(self,
        ...
        column_names = (
                ...
                [f'Wind Speed Interval:[{vwinds[i]}, {vwinds[i + 1]})'
                 for i in range(1, len(vwinds) - 1)] +
        ...
        windzero_frequency = windzero_frequency * (windzero_frequency /
    ...
    def process_season_data(self, ndirections, vwinds, season_df, filename):
        ...
        if 99.9999 <= count_df.iloc[-1, -1] < 100.0001:
            table = count_df.drop(index=['No Direction', 'Total'],
                                  columns=['total'])
