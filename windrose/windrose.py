...
class WindroseAnalysis:
    ...
    def calculate_wind_frequency(self, ndirections, vwinds, speed, dirc, direction):
        """
    ...
    def generate_frequency_table(self, ndirections, vwinds, speed, dirc, direction):
        """
        ...
        count = self.calculate_wind_frequency(ndirections, vwinds, speed, dirc, direction)
        count = np.hstack((count[:, [0]], np.diff(count, axis=1)))
        column_names = (
                [f'Wind Speed Interval:({vwinds[0]}, {vwinds[1]})'] +
                [f'Wind Speed Interval:[{vwinds[i]}, {vwinds[i + 1]})' for i in range(1, len(vwinds) - 1)] +
        ...
        center_angle = np.linspace(0, 360, ndirections + 1)[:-1]
        directions = np.mod(self.north - center_angle / 90 * (self.north - self.east), 360)
        n = 180 / ndirections
        ...
        windzero_frequency = windzero_frequency * (windzero_frequency / 100 > np.finfo(float).eps)
        count_df.loc['No Direction'] = (['[0 , 360)', 'Wind Speed = 0'] + [''] * (count_df.shape[1] - 3)
    ...
    def process_season_data(self, ndirections, vwinds, season_df, filename):
        ...
        dirc = np.mod((self.north - direction) / (self.north - self.east) * 90, 360)
        ...
        count_df = self.generate_frequency_table(ndirections, vwinds, speed, dirc, direction)
